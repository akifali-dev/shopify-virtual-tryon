...
export async function uploadBase64ImageHelper(
  base64,
  mimeType,
  fileNameHint = "output.png",
) {
  if (!base64) throw new Error("No base64 provided");
  const buffer = Buffer.from(base64, "base64");
  const safeName = `uploads/${Date.now()}-${removeWhiteSpaces(fileNameHint)}`;
  const url = await storeFile(buffer, mimeType || "image/png", safeName);
  return url;
}
