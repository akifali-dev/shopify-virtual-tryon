...
## Configuring Try-On Image Metafield

The Virtual Try-On extension uses the product's first image when no additional image is specified. To choose a different image:

1. From your Shopify admin, go to **Settings > Custom data > Products**.
2. Add a new product metafield with the namespace `custom` and key `tryon_image` and select the **File** type.
3. For each product, upload or select the image you want to use in this metafield.

If the metafield is left empty, the app falls back to the product's first media image.
